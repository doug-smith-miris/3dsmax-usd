...
#### Fixes:
- **MAX-MTLX-012** Author bump/normal-map STRENGTH scalar on `ND_normalmap_float.scale` (MaterialX 1.38+) / `ND_normalmap_vector2.scale` from the Max material's bump-strength property. `MtlxIOUtil.ExportMtlxString` had been dropping this input the same way it drops `.in` (which MAX-MTLX-003 already restores), so every scaffolded normalmap ran at its port default `1.0` regardless of the artist-authored scalar on the source material. Adds `_ApplyBumpStrengthToNormalmaps` + `discoverMaxMtlxBumpStrengthFn` to `src/translators/MtlxShaderWriter.cpp`, wired at the export site after `_WireDanglingNormalmapInputs` and before `_AddDependentNodes`. Property table (probed in order per sub-material): `bump_map_amt` (PhysicalMaterial), `bumpMapAmount` (OpenPBR / camelCase PhysicalMaterial), `bump_multiplier` (VRayMtl), `bumpAmount` (StdMaterial legacy). Discovery reuses the MAX-MTLX-007 `unwrapBlendMaterialSubMtls` wrapper walk so `VRayBlendMtl` / `VRayOverrideMtl` unwrap base-first (baseMtl's strength wins over any coat's or per-ray override's, matching layered-material precedence). No-op contract: (a) no normalmap nodes in the shader → skip MAXScript round-trip; (b) no probeable strength property on any walked sub-material → return 0; (c) discovered value equals port default 1.0 within 1e-6 → return 0 (doc stays clean of synthetic `scale=1`); (d) non-numeric property value (e.g. Texmap in the same-name slot on an unfamiliar wrapper) → skip and try next; (e) negative artist value → clamped to 0. Adds `src/Tests/Integration/test_miris_max_mtlx_012.py` (33 tests: pre-fix defect visibility, per-property dispatch across all four spellings, wrapper walk with blend-base-wins + override-base-wins + nested-blend-of-blend, port-default skip at exact 1.0 and near-1.0 epsilon, non-numeric skip with fallthrough, negative clamp, multi-normalmap authoring, no-normalmap early-out, surgical scope — only `scale` authored, `in`/`normal`/`tangent`/`bitangent`/`file` untouched, idempotent re-apply, walker-reuses-MTLX-007 with VRayMtlWrapper / MultiSub not descended, MaterialX standard-library `scale` default anchored at 1.0, and a 179-material arena-density census invariant), and the `MAX-MTLX-012` row + notes section + changelog bullet in `doc/translation-mapping.md`. Structural-only fix — pixel-level verification requires the Windows-box rebuild + real Max scene export path (follow-on operational bite tracked by `vray-arch-verifier.ts`'s `coverageVerdict`); the 33-test hython mirror is the correctness proof for this bite.
- **MAX-MTLX-008** MTLX wrapper-walk scope-coverage audit: locks in the MAX-MTLX-007 `unwrapBlendMaterialSubMtls` scope (`VRayBlendMtl` with baseMtl + coatMtl\_1..coatMtl\_9; `VRayOverrideMtl` with baseMtl / giMtl / reflectMtl / refractMtl / shadowMtl; baseMtl-first ordering; class-name gate EXACT) with a 48-case negative-test suite so a wildcard refactor fails by named case. Adds `src/Tests/Integration/test_miris_max_mtlx_008.py` (class-name gate rejects superstring / substring / different-suffix; case-insensitive `==` semantics documented; six adjacent V-Ray wrappers explicitly rejected — VRay2SidedMtl / VRayMtlWrapper / VRayFastSSS2 / VRayLightMtl / VRayBumpMtl / VRayHairMtl; seven stock 3ds Max wrappers explicitly rejected — Multi/Sub-Object / DoubleSided / Shell Material / stock Blend / Composite Mtl / Top/Bottom / Matte/Shadow; `.baseMtl` collision-shape anchor with VRayOverrideMtl walked vs VRayMtlWrapper / VRayBumpMtl not walked; slot-table completeness — exactly 10 slots for VRayBlendMtl and exactly 5 for VRayOverrideMtl, novel slots rejected; slot-order load-bearing differential; idempotence + depth-cap + cycle safety; 189-material arena census with 89-material MAX-MTLX-007 delta invariant + 10 adjacent-wrapper negatives; cross-wrapper divergence anchor), a surgical-bounds comment block in `src/translators/MtlxShaderWriter.cpp` (inside `discoverMaxMtlxTexmapsFn`), and the `MAX-MTLX-008` row + notes section + collision-shape table in `doc/translation-mapping.md`. No C++ logic change — the audit's value is guardrailing what MAX-MTLX-007 established. The auto-planner's follow-on rationale ("VRayOverrideMtl (GImtl wrapper) baseMtl not unwrapped on standard map-discovery path") describes the pre-MAX-MTLX-007 baseline that MAX-MTLX-007 already fixed; the audit prevents a future "unify wrapper handling" refactor from silently descending into VRayMtlWrapper / VRayBumpMtl and double-counting substrate textures on the arch-viz arena baseline. The wildcard widening the literal reading would encode ("descend anything with `.baseMtl`") is exactly what this audit shape prevents.
